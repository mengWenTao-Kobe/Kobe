...
// 引入添加商品请求函数
import { addGoods, goodsEdit } from "@/api/goods.js";
// 引入session工具函数
...
export default {
  ...
  created() {
    ...
    this.ruleForm.goodscategory = row.goodscategory;
    this.ruleForm.goodsfeature = JSON.parse(row.goodsfeature);
    this.ruleForm.imgUrl = row.imgUrl;
    this.ruleForm.isPromotion = row.isPromotion;
    // 规格处理
    let standard = JSON.parse(row.standard);
    // 打印看看是单规格还是多规格
    // console.log(standard[0].goodsstandard);
    if (standard[0].goodsstandard == "单规格") {
      this.ruleForm.goodsstandard = "单规格";
      this.ruleForm.packingexpense = standard[0].packingexpense;
      this.ruleForm.goodsPrice = standard[0].goodsPrice;
    } else {
      this.ruleForm.goodsstandard = "多规格";
      this.tableData = standard;
    }
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(async valid => {
        if (valid) {
          //如果通过前端验证
          console.log(data);
          let { code, msg } = await goodsEdit(data);
          if (code === 0) {
